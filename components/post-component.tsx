...
const PostComponent = ({ post }: Props) => {
  return (
    <Card key={post?._id} className='overflow-hidden border-primary border rounded-xl shadow-primary group'>
    <Link href={`/posts/${post?.slug?.current}`} className='block group-hover:bg-primary/85'>
        <CardHeader className={`${font.className} border-b-2 border-primary flex flex-row justify-between items-end py-1.5 text-sm text-right`}>
